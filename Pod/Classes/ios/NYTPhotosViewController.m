...
#define kPhoneScreenWidth [[UIScreen mainScreen] bounds].size.width
#define kPhoneScreenHeight [[UIScreen mainScreen] bounds].size.height
...
- (void)viewDidLoad
{
    ...
    visualEffectView.frame = CGRectMake(0, 0, kPhoneScreenWidth, kPhoneScreenHeight);
...
- (void)viewWillLayoutSubviews
{
    ...
    self.overlayView.frame = CGRectMake(0, 30, self.view.frame.size.width, self.view.frame.size.height - 30);
}
...
- (instancetype)initWithPhotos:(NSArray <id <NYTPhoto>> * _Nullable)photos showWaterMark:(BOOL)showWatermarkImage {
    self.showWatermarkImage = showWatermarkImage;
    return [self initWithPhotos:photos];
}
...
- (NYTPhotoViewController *)newPhotoViewControllerForPhoto:(id<NYTPhoto>)photo
{
    if (photo)
    {
        ...
        photoViewController.showWatermarkImage = self.showWatermarkImage;
